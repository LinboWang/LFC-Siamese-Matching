...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad():
        for test_data in loader_iter:
            ...
            y_hat, e_hat = res_logits[-2], res_e_hat[-2]  # modify the index from -1 to -2 for reverse loss
            loss, geo_loss, cla_loss, l2_loss, prec, rec = match_loss.run(cur_global_step, test_data, y_hat, e_hat, toggle=False)
...
def test(data_loader, model, config):
    ...
    # save_file_best = os.path.join(config.model_path)
