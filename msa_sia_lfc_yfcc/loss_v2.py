...
class MatchLoss(object):
    ...
    def run(self, global_step, data, logits, e_hat, toggle=False):
        ...
        if toggle:
            e_gt = e_gt[:, [0, 3, 6, 1, 4, 7, 2, 5, 8]]  # For test reverse loss
        ...
        if toggle:
            pts2_virts, pts1_virts = pts_virt[:, :, :2], pts_virt[:, :, 2:]  # For test reverse loss
        else:
