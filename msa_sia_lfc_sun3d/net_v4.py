...
from loss_v2 import batch_episym
import numpy as np
...
class DGCNN_Block(nn.Module):
    ...
    def forward(self, features):
        ...
        local_feat = self.attn(out)
        atten_weights = self.atten_weight_conv(features.permute(0, 2, 3, 1))
        atten_weights = atten_weights.view(B, N, self.group, -1)
        atten_weights = F.softmax(atten_weights, dim=-1)
        local_feat = local_feat.view(B, self.group, -1, N, self.knn_num).permute(0, 3, 1, 4, 2)
        local_feat = torch.einsum("...i,...ij->...j", [atten_weights, local_feat])
        local_feat = local_feat.permute(0, 2, 3, 1).reshape(B, -1, N, 1)
...
class OANBlock(nn.Module):
    ...
    def forward(self, data, xs, toggle=False):
        # data: b*c*n*1
        if toggle:
            if data.shape[1] == 4:
                data = data[:, [2, 3, 0, 1], :, :]
            else:
                data = data[:, [2, 3, 0, 1, 4, 5], :, :]
            xs = xs[:, :, :, [2, 3, 0, 1]]
        ...
        x2, x1 = xs[:, 0, :, :2], xs[:, 0, :, 2:4]  # For test reverse loss
        e_hat_norm = e_hat
...
class OANet(nn.Module):
    ...
    def forward(self, data):
        ...
        logits, e_hat, residual = self.weights_init(input, data['xs'], toggle=False)
        res_logits.append(logits), res_e_hat.append(e_hat)
        for i in range(self.iter_num):
            logits_1, e_hat, residual_1 = self.weights_iter[i](
                torch.cat([input, residual.detach(), torch.relu(torch.tanh(logits)).reshape(residual.shape).detach()],
                          dim=1), data['xs'], toggle=False)
            res_logits.append(logits_1), res_e_hat.append(e_hat)
            logits, e_hat, residual = self.weights_iter[i](
                torch.cat([input, residual.detach(), torch.relu(torch.tanh(logits)).reshape(residual.shape).detach()],
                          dim=1), data['xs'], toggle=True)
            res_logits.append(logits), res_e_hat.append(e_hat)
